...
const NotificationDropdown = ({ notifications, setNotifications, setShowLocationModal }) => {
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex justify-center items-center h-12 w-12 relative ml-2 p-2 text-gray-600 hover:text-gray-800 focus:outline-none bg-white/60 hover:shadow-lg hover:shadow-gray-100/30 rounded-full hover:bg-white"
      >
        <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" width={48} height={48} viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}><g><path strokeDasharray={4} strokeDashoffset={4} d="M12 3v2"><animate fill="freeze" attributeName="stroke-dashoffset" dur="0.2s" values="4;0"></animate></path><path strokeDasharray={28} strokeDashoffset={28} d="M12 5c-3.31 0 -6 2.69 -6 6l0 6c-1 0 -2 1 -2 2h8M12 5c3.31 0 6 2.69 6 6l0 6c1 0 2 1 2 2h-8"><animate fill="freeze" attributeName="stroke-dashoffset" begin="0.2s" dur="0.4s" values="28;0"></animate></path><animateTransform fill="freeze" attributeName="transform" begin="0.9s" dur="6s" keyTimes="0;0.05;0.15;0.2;1" type="rotate" values="0 12 3;3 12 3;-3 12 3;0 12 3;0 12 3"></animateTransform></g><path strokeDasharray={8} strokeDashoffset={8} d="M10 20c0 1.1 0.9 2 2 2c1.1 0 2 -0.9 2 -2"><animate fill="freeze" attributeName="stroke-dashoffset" begin="0.6s" dur="0.2s" values="8;0"></animate><animateTransform fill="freeze" attributeName="transform" begin="1.1s" dur="6s" keyTimes="0;0.05;0.15;0.2;1" type="rotate" values="0 12 8;6 12 8;-6 12 8;0 12 8;0 12 8"></animateTransform></path></g></svg>
        {notifications.filter(n => !n.read).length > 0 && (
          <span className="animate-bounce absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg py-2 z-50">
          <div className="px-4 py-3 border-b border-gray-100 mb-3 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-800 font-display !mb-0">Bildirimler</h3>

            <button className='text-gray-500 hover:text-gray-700 rounded-lg ml-4' onClick={refreshNotifications}>
            <svg xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24"><path fill="currentColor" d="M17.65 6.35A7.96 7.96 0 0 0 12 4a8 8 0 0 0-8 8a8 8 0 0 0 8 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0 1 12 18a6 6 0 0 1-6-6a6 6 0 0 1 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4z"></path></svg>
            </button>
          </div>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      {notification.submessage && (
                        <p className="text-xs text-orange-600 px-2 py-1 rounded-lg bg-orange-100/90 mt-2 inline-block">
                          {notification.submessage}
                        </p>
                      )}
                    </div>
