...
def create_authorization_udf(post_owner, applicant, post, application, price):
    ...
    # YETKİLİ KILINAN AVUKAT ismini ekle
    # Şablonda isim alanı yok, bu yüzden "Vergi Dairesi ve Sicil No" alanını "Adı Soyadı" olarak değiştiriyoruz.
    # UDF yapısını bozmamak için karakter sayısını (32) korumalıyız.
    applicant_name = applicant.get('name', '')
    target_field = 'Vergi Dairesi ve Sicil No\t   : \t'
    
    if applicant_name:
        # "Adı Soyadı" etiketi ve isim
        # Orijinal alan 32 karakter uzunluğunda.
        prefix = 'Adı Soyadı\t\t: '
        new_content = f"{prefix}{applicant_name}"
        
        # 32 karaktere tamamla (padding)
        if len(new_content) < 32:
            new_content += ' ' * (32 - len(new_content))
            
        filled_text = filled_text.replace(target_field, new_content, 1)
