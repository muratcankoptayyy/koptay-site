...
def _get_secret_key() -> str:
    """Get or generate a secure secret key with validation."""
    import secrets
    
    key = os.getenv("FLASK_SECRET_KEY")
    is_dev = _coerce_bool(os.getenv("DEV_MODE"), default=False) or os.getenv("FLASK_ENV", "").lower() in {"development", "dev"}
    
    # Production'da weak key kontrolü
    if not is_dev:
        weak_keys = ["dev-secret-key", "change-in-production", "secret", "key"]
        if not key or any(weak in key.lower() for weak in weak_keys):
            raise ValueError(
                "\n⚠️  CRITICAL ERROR: Production'da güçlü SECRET_KEY gerekli!\n"
                "Şunu çalıştırın:\n"
                "  python -c 'import secrets; print(secrets.token_hex(32))'\n"
                "Ve çıktıyı .env dosyasına FLASK_SECRET_KEY olarak ekleyin.\n"
            )
        
        if len(key) < 32:
            raise ValueError(
                f"\n⚠️  CRITICAL ERROR: SECRET_KEY çok kısa ({len(key)} karakter)!\n"
                "En az 32 karakter olmalıdır.\n"
            )
    
    # Development için otomatik generate
    if not key:
        # Always generate for dev/test automatically
        key = secrets.token_hex(32)
        print(f"⚠️  Auto-generated SECRET_KEY for development: {key[:16]}...")
...
def get_config() -> Type[Config]:
    ...
    # Prefer explicit FLASK_ENV, but allow DEV_MODE=1 as a convenience for local runs
    env = os.getenv("FLASK_ENV", "production").lower()
    if env in {"development", "dev"}:
        return DevelopmentConfig
    if _coerce_bool(os.getenv("DEV_MODE"), default=False):
        return DevelopmentConfig
    return Config
